                ...
                wh_callback_t *cb)
{
        ...
        status =value_list_to_json(cb->send_buffer,
                        cb->send_buffer_size,
        ...
        status = wh_flush_nolock (0, cb);
        pthread_mutex_unlock (&cb->send_lock);
   
        DEBUG ("write_http plugin: <%s> buffer %zu",
                        cb->location,
                        cb->send_buffer_size);
                ...
                wh_callback_t *cb)
{
        ...
        /* Check if we have enough space for this command. */        pthread_mutex_unlock (&cb->send_lock);
        pthread_mutex_unlock (&cb->send_lock);
        return (0);
...
static int wh_config_node (oconfig_item_t *ci) /* {{{ */
{
		...
		DEBUG("Done with plugin_register_write");
